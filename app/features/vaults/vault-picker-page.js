...
/**
 * @typedef {Object} VaultPickerProps
 * @property {Array<Object>} vaults
 * @property {function} onCreateVault
 * @property {function(Object): void} onSelectVault
 */
    ...
    return {
        ...
        render(container) {
            ...
            const vaultListHtml = vaults.length
                ? `<div class="vault-list vault-home__list">${vaults.map((v) => vaultCardHtml(v)).join("")}</div>`
                : emptyStateHtml({
                    title: "No Vaults Yet",
                    message: "This wallet has not created a local vault on this device yet.",
                    iconSrc: "./assets/brand/SymbolLogo.svg",
                    primaryActionHtml: buttonHtml({ label: "Create Your First Vault", tone: "primary", dataAction: "create-vault" }),
                });
            ...
            page.querySelectorAll("[data-action='create-vault']").forEach((btn) => {
                btn.addEventListener("click", () => onCreateVault());
            });

            page.querySelectorAll("[data-action='open-vault']").forEach((btn) => {
                const card = btn.closest("[data-vault-id]");
                const vaultId = card?.dataset.vaultId;
                const vault = vaults.find((v) => v.id === vaultId);
                if (vault) {
                    btn.addEventListener("click", () => onSelectVault(vault));
                }
            });
